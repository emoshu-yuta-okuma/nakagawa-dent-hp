...
<form id="user_add" action="?m=$form{'m'}&a=save" method="POST" onSubmit="return checkUserEdit(this,'${submit}');">
	<table cellpadding="0" cellspacing="0" class="sheet">
		...
		<tr>
			<th>通し番号</th>
			<td><input type="checkbox" name="serials" value="1"${serial_checked} /> 件名に通し番号を付ける ／ 現在の通し番号 <input type="text" name="serial_num" value="${serial_num}" size="5" style="text-align: right;ime-mode: disabled;" /></td>
		</tr>
		<tr>
			<th $site_url_error_bg>URL（トップページ）</th>
			<td $site_url_error_bg>
				<span class="error_block">$site_url_error_msg</span>
				<input type="text" name="site_url" value="${site_url}" size="40" />
			...
			<td>
				<table class="table_style_none">
					<tr>
						...
						<td>
							　　設定値：
							<input type="text" name="mailform_sender_address_name" value="${mailform_sender_address_name}" size="30" id="mailform_sender_address_name" />
							（手動設定可能）
							</td>
					</tr>
					<tr>
						...
						<td>
							　　設定値：
							<input type="text" name="mailform_sender_address" value="${mailform_sender_address}" size="30" id="mailform_sender_address" onblur="email_check(this.id)" />
							（手動設定可能）
						</td>
					</tr>
				</table>
		...
		<tr>
			...
			<td $client_info_error_bg>
				...
				<textarea name="client_info" style="width: 100%;height: 100px;">${client_info}</textarea>
		...
		<tr>
			...
			<td>
				...
				URLの指定が無い場合は標準サンクスページ(fmail.cgi?mode=thanks)が表示されます。<br />
				<span class="caution">
					指定する場合は、相対パスか絶対パスかで記載。相対パスの場合、fmailフォルダ直下が現在位置となる。<br />
					<strong>例：./thank.html　（この場合は、./fmail/thank.htmlにファイルを置く。）</strong>
				</span>
				ただし、殆どの場合、fmail.tplで解決できます。
			</td>
		</tr>
		<tr>
			<th>サンクスメッセージ</th>
			<td>
				<textarea name="thanks_message" style="width: 100%;height: 100px;">${thanks_message}</textarea><br />
				<span class="caution">※fmail.cgi?mode=thanksのサンクスページにのみ表示されます</span>
			</td>
		</tr>
		<tr>
			<th>sendmailパス</th>
		...
		<tr>
			...
			<td>
				...
				対象：textarea （標準：引継なし）<br />
				<input type="checkbox" name="cart_logsave" value="1"${cart_logsave_checked} id="cart_log_save" /> サーバに商品カートの履歴を保存する。
			</td>
		</tr>
		...
		<tr>
			<th>スパムフィルタ</th>
			<td><input type="checkbox" name="spamcheck" value="1"${spamcheck_checked} /> 入力内容がすべて英語の場合や<strong>\[/link\]</strong>や<strong>\[/url\]</strong>という文字が含まれる場合、エラーにする</td>
		</tr>
		<tr>
			<th>未入力項目</th>
			<td>
				<input type="checkbox" name="mail_dustclear" value="1"${mail_dustclear_checked} /> 未入力項目をメールに反映しない。<br />
				<input type="checkbox" name="mail_dustclear_zero" value="1"${mail_dustclear_zero_checked} /> 「0」または、「0個」をメールに反映しない。
		...
		<tr>
			...
			<td>
				<span>入力画面</span>
				<input type="text" name="title_mailform" id="title_mailform" value="${title_mailform}" size="40" />
				<span>確認画面</span>
				<input type="text" name="title_confirm" id="title_confirm" value="${title_confirm}" size="40" />
				<span>エラー画面</span>
				<input type="text" name="title_error" id="title_error" value="${title_error}" size="40" />
				<span>完了画面</span>
				<input type="text" name="title_thanks" id="title_thanks" value="${title_thanks}" size="40" />
			</td>
		</tr>
		<tr>
			<th>アフィリエイト</th>
		...
		<tr>
			...
			<td>
				...
				<input type="text" name="afiri_tag" id="afiri_tag" value="${afiri_tag}" class="text_long" /><br />
		...
		<tr>
			...
			<td>
				...
				<span class="caution">（チェック時のみ「fmail_smartphone.tpl」がスマートフォンでの閲覧時に反映します。）</span>
			</td>
		</tr>
		<tr>
