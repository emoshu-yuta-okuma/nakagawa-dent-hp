...
if($_GET{'mode'} eq "send" && -f $send_token){
	...
	for($cnt=0;$cnt<@elements;$cnt++){
		($elements_id,$num,$name,$type_of_element,$html_size,$html_rows,$html_cols,$html_id,$element_type,$check_type,$on_event,$html_tag_free,$text_min,$text_max,$enable_filetypes,$filesize_min,$filesize_max,$checked_min,$checked_max,$element_valus,$element_text,$html_example,$note,$element_error_message,$must_disp,$default_value,$system_disp_false,$html_tag_free_top,$elements_class,$smartphone_elements_type) = split(/\t/,$elements[$cnt]);
		#項目名の改行処理
		$name =~ s/&lt;-br-&gt;//g;
	...
	for($cnt_rmail=0;$cnt_rmail<@return_mail_set;$cnt_rmail++){
		...
		if($return_mail_type){
			...
			for($cnt=0;$cnt<@join_elements;$cnt++){
				($elements_id,$num,$name,$type_of_element,$html_size,$html_rows,$html_cols,$html_id,$element_type,$check_type,$on_event,$html_tag_free,$text_min,$text_max,$enable_filetypes,$filesize_min,$filesize_max,$checked_min,$checked_max,$element_valus,$element_text,$html_example,$note,$element_error_message,$must_disp,$default_value,$system_disp_false,$html_tag_free_top,$elements_class,$smartphone_elements_type) = split(/\t/,$join_elements[$cnt]);
				$elementname = "en${elements_id}";
				@values = $q->param($elementname);
				$values = join("\n",@values);
				$safe = 100;
...
#			Jcode::convert(\$send_subject,"sjis","utf8");
			...
			if(1 == grep(/^${send_conditional_value}$/,@values)){
				...
				for($cnt2=0;$cnt2<@elements;$cnt2++){
					($elements_id,$num,$name,$type_of_element,$html_size,$html_rows,$html_cols,$html_id,$element_type,$check_type,$on_event,$html_tag_free,$text_min,$text_max,$enable_filetypes,$filesize_min,$filesize_max,$checked_min,$checked_max,$element_valus,$element_text,$html_example,$note,$element_error_message,$must_disp,$default_value,$system_disp_false,$html_tag_free_top,$elements_class,$smartphone_elements_type) = split(/\t/,$elements[$cnt2]);
					if($join_values{$elements_id}){
						$send_body =~ s/&lt;${elements_id}&gt;/$join_values{$elements_id}/g;
						#件名置換
						$send_subject =~ s/&lt;${elements_id}&gt;/$join_values{$elements_id}/g;
				...
				if($charset ne 'utf8'){
					...
					$send_mail_body = &line1000Bytes($send_mail_body);
				}
...
#		($conditional_id,$must_name,$error_message,$must_elements,$note) = split(/\t/,$must[$cnt]);
	...
	if($send_flag > 0){
		...
		if($flag_mua == 1){
			# 送信ボタン
			$contents .= "<form id=\"fmail_submit\" class=\"fmail_submit\" method=\"post\" action=\"fmail.cgi?mode=send${sesQuery}\">\n";
			...
			# 戻るボタン
			$contents .= "<form id=\"fmail_cancel\" class=\"fmail_cancel\" method=\"post\" action=\"fmail.cgi?mode=back\">\n";
			$contents .= "<div class=\"hiddenObj\">";
			if($flag_js){
				#JS利用可能の場合
...
#	$scriptObj = "\n<script type=\"text/javascript\">\n\nvar exampleObj = new Array();\nvar postvalueObj = new Array();\nvar elementsetObj = new Array();\n";## fix / keep selectObj 090723
	$contents = "\n<form id=\"fmail\" method=\"post\" action=\"fmail.cgi?mode=confirm${sesQuery}\"${enctype} onsubmit=\"return falsesubmit(this)\">\n";
	...
	for($cnt=0;$cnt<@elements;$cnt++){
		...
		if($html_size ne $null){
			$html_size = " size=\"${html_size}\"";
		}
		if($on_event ne $null){
			$on_event = " ${on_event} ";
		}
		...
		if($element_type eq "text"){
			...
			$elementtag = "${disp_error_message}${html_tag_free_top}<input type=\"$text_format\" name=\"en${elements_id}\" id=\"en${elements_id}\"${eclass} value=\"${values[0]}\"${html_size}${on_event} />${note}\n";
		}
			...
			$elementtag = "${disp_error_message}${html_tag_free_top}<input type=\"hidden\" name=\"en${elements_id}\" id=\"en${elements_id}\"${eclass} value=\"${values[0]}\"${html_size}${on_event} />${note}\n";
		}
			...
			if($display_element_flag){
				...
				#スマフォアクセスの場合
					...
					if(${elementtag} !~ /type="file"/){
						...
						$contents .= "<div id=\"p_en${elements_id}\" class=\"fmail_item$fmail_item_color$add_class\">\n";
						$contents .= "<p${classname} id=\"h_en${elements_id}\" class=\"fmail_item_name\"><b>${name}</b>${must_icontag}</p>\n";
						$contents .= "<p${classname} id=\"d_en${elements_id}\" class=\"fmail_item_value\">${elementtag}${html_tag_free}</p>\n";
						$contents .= "</div>\n";
					}
				#talbe定義の場合
					...
					# Classが追加されている場合
					if($add_class) {
						$add_class = qq|class="$add_class"|;
					}
					$contents .= "<tr id=\"r_en${elements_id}\" $add_class>\n";
					$contents .= "<th${classname} id=\"h_en${elements_id}\">${must_icontag}${name}</th>\n";
